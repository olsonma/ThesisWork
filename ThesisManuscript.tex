...
\section{Unplanned Sample Sizes in First Stage}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Because accruement of patients can often be unexpected in the first stage, it's imperative that methods are available to handle situations with attained sample sizes that differ from the planned sample size. Chang \textit{et al} \cite{Chang} proposed an alternative design that is an extension of Simon’s two stage design to handle unplanned sample sizes in both the first and second stages. Because it's desired to stay as closely to the original design as possible, we investigate this method using only attained first stage sample sizes while maintaining the original second stage sample size or original total sample size. Let $n_1$, $n_t$, $r_1$, $r_t$, $p_0$, $p_1$, $\alpha$, and $\beta$ be the original, planned design parameters \textbf{can we call these design parameters? Jung calls design parameters p0,p1,alpha,beta} and let $n_1^\ast$ be the attained sample size in the first stage, $n_t^\ast$ be the attained sample size at the end of the second stage, and $n_2^\ast = n_t^\ast - n_1^\ast$.  Chang \textit{et al} proposes that type II error probability spent in stage I, based on planned and attained sample size, is given by $\beta_1 = \mbox{P}(X_1 \leq r_1 \vert n_1, p = p_1)$ \\
...
\begin{itemize}
\item Somewhere we want to explain that we want to keep either nt or n2 the same to stick as close to the orginal design as we can. Sticking to nt have properties closest to the original design- will have to look into this more and possibly with a more formal proof. I'm not sure what happens if both stages are different - this would get more complicated in prespecifying a design. I think Jeffrey's keeps the total sample size the same. 
\item Talk about when the adaptation design and chang design will differ. (extreme sample size shifts? I cant remember.)
\end{itemize}

\begin{itemize}
\item Green and Dahlberg review adjusting stage I and II sample sizes based on other designs (gehan, fleming, oncology group) but they say that simons is difficult to adjust. page 857 of green and dahlberg. 
\item Chang is comparing his method to the SWOG in Green and Dahlbergs... can we use this for simon's design?
